...
} from "@/components/plan/historical-chart";
...
export function HistoricalTab({
  ...
  React.useEffect(() => {
    ...
    fetchSchemeNav(schemeCode)
      ...
      .catch(() => {
        if (!cancelled)
          setError(
            "Couldn't load NAV history for this fund. Try again in a moment.",
          );
      });
    return () => {
      cancelled = true;
      ...
      .then((data) => {
        ...
        if (plan.startDate < first || plan.startDate >= last) {
          ...
          const events = plan.events.map((e) =>
            e.type === "SIP_START" && e.date === previousStart
              ? { ...e, date: suggestedStart }
              : e,
          );
          onChange({
            ...plan,
            historicalScheme: scheme,
  ...
  function handleClearFund() {
    onChange({ ...plan, historicalScheme: undefined });
  }
  ...
  // --- Multi-fund path --------------------------------------------------
  const [navByAllocationId, setNavByAllocationId] = React.useState<
    Map<string, SchemeNavData>
  >(new Map());
  ...
  const allocationKey = isPortfolio
    ? allocations!
        .map((a) => `${a.id}:${a.historicalScheme!.schemeCode}`)
        .join(",")
    : "";

  const [prevAllocationKey, setPrevAllocationKey] =
    React.useState(allocationKey);
  ...
  React.useEffect(() => {
    ...
    Promise.all(
      allocations.map((a) =>
        fetchSchemeNav(a.historicalScheme!.schemeCode).then(
          (data) => [a.id, data] as const,
        ),
      ),
    )
      .then((entries) => {
      ...
      .catch(() => {
        if (!cancelled)
          setPortfolioError(
            "Couldn't load NAV history for one or more funds. Try again in a moment.",
          );
      });
    return () => {
      cancelled = true;
  ...
  const portfolioDataReady =
    isPortfolio &&
    !!allocations &&
    allocations.every((a) => navByAllocationId.has(a.id));
  const portfolioLoading =
    isPortfolio && !portfolioDataReady && !portfolioError;
  ...
  React.useEffect(() => {
    ...
    if (plan.startDate < latestFirst || plan.startDate >= earliestLast) {
      ...
      const suggestedStart =
        tenYearsBack > latestFirst ? tenYearsBack : latestFirst;
      const previousStart = plan.startDate;
      const events = plan.events.map((e) =>
        e.type === "SIP_START" && e.date === previousStart
          ? { ...e, date: suggestedStart }
  ...
  const portfolioWindow = React.useMemo(() => {
    ...
    const navMap = new Map(
      allocations.map((a) => [a.id, navByAllocationId.get(a.id)!.navs]),
    );
  ...
  const portfolioResult = React.useMemo(() => {
    if (!portfolioWindow || portfolioWindow.fatalError || !allocations)
      return null;
    const navMap = new Map(
      allocations.map((a) => [a.id, navByAllocationId.get(a.id)!.navs]),
    );
    return simulatePortfolioHistorical(
      plan,
      allocations,
      navMap,
      portfolioWindow,
  ...
  // --- Combined view ------------------------------------------------------
  const activeResult = isPortfolio
    ? (portfolioResult?.overall ?? null)
    : directResult;
  const activeStartDate =
    isPortfolio && portfolioWindow ? portfolioWindow.startDate : plan.startDate;
  const fundName = isPortfolio ? "your portfolio" : (navData?.schemeName ?? "");

  const projectedFromDate = !isPortfolio
    ? (directResult?.projectedFromDate ?? null)
    : null;

  const chartData: HistoricalChartPoint[] = React.useMemo(() => {
    if (!activeResult) return [];
    const start = parseISO(activeStartDate);
    const splitIdx = projectedFromDate
      ? activeResult.ledger.findIndex((r) => r.date === projectedFromDate)
      : -1;
    ...
    const realBoundaryIdx =
      splitIdx >= 0 ? splitIdx - 1 : activeResult.ledger.length - 1;
    return activeResult.ledger.map((row, i) => {
      ...
      return {
        ...
        projectedValue:
          splitIdx >= 0 && i >= realBoundaryIdx ? row.value : undefined,
        real: realValue(
          row.value,
          plan.assumptions.inflation,
          Math.max(years, 0),
        ),
        regularValue: !isPortfolio
          ? regularResult?.ledger[i]?.value
          : undefined,
      };
    });
  }, [
            ...
            <Card>
              <CardContent className="flex flex-col items-center gap-2 py-12 text-center">
                ...
                <p className="max-w-sm text-sm text-muted-foreground">
                  Search for a real mutual fund on the right, and this plan
                  replays against what actually happened to it — real NAV, not
                  an assumption.
                </p>
              </CardContent>
            </Card>
          ...
          {activeResult && !loading && !portfolioLoading && (
            <>
              ...
              {isPortfolio && portfolioResult && (
                <PortfolioBreakdownTable legs={portfolioResult.legs} />
              )}
              <LedgerTable
                ledger={activeResult.ledger}
                legs={portfolioResult?.legs.map((l) => ({
                  label: l.allocation.label,
                  ledger: l.result.ledger,
                }))}
              />
            </>
          )}
          ...
          {!isMultiFund && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Real AMFI-sourced NAV, via a free community API.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3">
                <FundSearch
                  selected={plan.historicalScheme ?? null}
                  onSelect={handleSelectFund}
                  onClear={handleClearFund}
                />
                {navData && firstIso && lastIso && (
                  <p className="text-xs text-muted-foreground">
                    Data available {formatDate(firstIso)} –{" "}
                    {formatDate(lastIso)}.
                  </p>
                )}
                {startBeforeInception && firstIso && (
                  <Warning>
                    Your plan starts before this fund&apos;s data begins (
                    {formatDate(firstIso)}) — earlier months use the inception
                    NAV.
                  </Warning>
                )}
                {directResult?.isProjected &&
                  directResult.projectedFromDate &&
                  directResult.projectedReturnPct != null && (
                    <Warning>
                      Real data ends{" "}
                      {formatDate(lastIso ?? directResult.projectedFromDate)} —
                      from {formatDate(directResult.projectedFromDate)} onward,
                      this plan is projected using {fundName || "this fund"}
                      &apos;s own trailing CAGR of{" "}
                      {directResult.projectedReturnPct.toFixed(1)}% /yr. Not a
                      guarantee — just its own past, carried forward.
                    </Warning>
                  )}
              </CardContent>
            </Card>
          ...
          {isPortfolio && portfolioWindow && !portfolioWindow.fatalError && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  The stretch where every fund in the portfolio has real data.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3">
                <p className="text-xs text-muted-foreground">
                  {formatDate(portfolioWindow.startDate)} –{" "}
                  {formatDate(portfolioWindow.endDate)}
                </p>
            ...
            <CardContent className="grid gap-5">
              ...
              <PlanTimingField
                ...
                onChange={(startDate, endDate) =>
                  onChange({ ...plan, startDate, endDate })
                }
              />
              <SliderField
                id="hist-inflation"
                ...
                onChange={(v) =>
                  onChange({
                    ...plan,
                    assumptions: { ...plan.assumptions, inflation: v },
                  })
                }
                min={0}
                max={15}
